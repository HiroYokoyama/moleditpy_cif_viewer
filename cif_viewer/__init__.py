    ...
    def draw_ellipsoid_model(mw, mol):
        ...
        if hasattr(mw, "init_manager") and hasattr(mw.init_manager, "settings"):
            plotter.set_background(
                mw.init_manager.settings.get("background_color", "#919191")
            )
        ...
        mesh_props = dict(
            ...
            lighting=mw.init_manager.settings.get("lighting_enabled", True)
            if hasattr(mw, "init_manager")
            else True,
        )
        ...
        for index, atom in enumerate(widget.last_rendered_atoms):
            ...
            color_rgb = (
                [float(x) for x in col[index]] if index < len(col) else [0.5, 0.5, 0.5]
            )
                ...
                if symbol == "H":
                    ...
                    if pt is not None:
                        try:
                            ...
                            logging.debug(
                                "Failed to get VDW radius for H from periodic table: %s",
                                e,
                            )
