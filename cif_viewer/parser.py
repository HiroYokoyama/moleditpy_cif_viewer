...
def is_polymer_structure(structure: CifStructure, tolerance: float = 0.45) -> bool:
    """
    Return True if the asymmetric unit is part of a periodic polymer or
    framework (i.e., at least one covalent bond in the asymmetric-unit graph
    crosses a unit-cell boundary).

    This is used by the viewer to automatically switch away from
...
def grow_molecules(
    ...
    3. BFS with cumulative offset tracking → each molecule is contiguous and
       never split at a cell boundary.  For frameworks/polymers, BFS follows
       bonds across periodic images so the whole connected component is
       returned with consistent geometry.
    4. Keep BFS components that overlap the [0,1)^3 unit-cell region (i.e.,
       at least one atom has fractional coordinates already wrapped into
       [0,1)).  This replaces the old ``is_asym`` filter which silently
       discarded entire polymer components when none of the symmetry-expanded
       atoms happened to be generated by the exact identity operation.
    """
    core_atoms = (
        structure.asymmetric_atoms
        if structure.asymmetric_atoms is not None
